...
float TurnSpeedMultiplier = 0.65;
...
void setup()
{
  ...
  // move wrist very high and arm all the way down and then set variables
  /*Serial.print("ArmCurrPos: ");
  Serial.println(ArmCurrPos);
  Serial.print("WristCurrPos: ");
  Serial.println(WristCurrPos);*/
 ...
 void loop()                             //Loop continuously
 {
  ...
  if(heartbeatactive && (currentMillis - LastHeartBeat > HeartBeatInterval)) { //Expected HeartBeat was not received, stop Robot
    ...
    ArmTargetPos = ArmCurrPos;              //Stop Arm in case it is moving  // removed to see. // todo could put whole file into origibot and put private
  ...
  if (ArmTargetPos > ArmCurrPos){           //Arm needs to move down
    //ArmTargetPos = ArmMaxPos;
    ArmRelay.aWrite2(0);                  //Turn ON Arm Relay
